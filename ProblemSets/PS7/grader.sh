...
for (( i=0; i<${#names[@]}; i++ )); do
    ...
    # 1 -- free points
    points=5
    cd ${u}/DScourseS24/ProblemSets/PS7
    ...
    # 4 -- PS7_*.{R,py,jl} file
    for file in PS7_*.{R,py,jl,ipynb}; do
        if [[ -f $file ]]; then
            points=$((points + 30))
            echo "$file exists"
            cat $file
        else
            echo "No PS7_* script file"
        fi
    done

    # 5 -- .tex and .pdf file in PS7 folder
    for file in PS7_*.{tex,pdf,txt}; do
        ext="${file: -3}"
        if [[ -f $file ]]; then
            echo "$file exists"
            if [[ $ext != "pdf" ]]; then
            ...
            if [[ $ext == "pdf" ]]; then
                echo "No PS7_*.pdf writeup file"
            fi
            if [[ $ext != "pdf" ]]; then
                echo "No PS7_*.tex writeup file"
            fi
        fi
    done
...
cd ~/teaching/DScourseS24/ProblemSets/PS7
