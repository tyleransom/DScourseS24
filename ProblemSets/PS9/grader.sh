...
for (( i=0; i<${#names[@]}; i++ )); do
    ...
    # 1 -- free points
    points=5
    cd ${u}/DScourseS24/ProblemSets/PS9
    ...
    # 4 -- PS9_*.{R,py,jl,ipynb} file
    for file in PS9_*.{R,r,py,jl,ipynb}; do
        if [[ -f $file ]]; then
            points=$((points + 25))
            echo "$file exists"
            cat $file
        else
            echo "No PS9_* script file"
        fi
    done

    # 5 -- .tex and .pdf file in PS9 folder
    for file in PS9_*.{tex,pdf,txt}; do
        ext="${file: -3}"
        if [[ -f $file ]]; then
            echo "$file exists"
            if [[ $ext != "pdf" ]]; then
            ...
            if [[ $ext == "pdf" ]]; then
                echo "No PS9_*.pdf writeup file"
            fi
            if [[ $ext != "pdf" ]]; then
                echo "No PS9_*.tex writeup file"
            fi
        fi
    done
...
cd ~/teaching/DScourseS24/ProblemSets/PS9
